...
!!! attention
    通过运行 man pidstat 可知，pidstat 默认显示进程的指标数据，加上 -t 参数后，才会输出线程的指标。
...
中断，它只发生在内核态，我们可以从 /proc/interrupts 这个只读文件中读取中断的详细信息。
...
观察一段时间，可以发现，变化速度最快的是**重调度中断（RES）**，这个中断类型表示，唤醒空闲状态的 CPU 来调度新的任务运行。这是多处理器系统（SMP）中，调度器用来分散任务到不同 CPU 的机制，通常也被称为**处理器间中断（Inter-Processor Interrupts，IPI）**。
